...
// ==/UserScript==
(function() {
    'use strict';const FIXED_TOKEN = 'eyJhbGciOiJIUzI1NiJ9.eyJwYXNzY29kZUlkIjo4MDQ4LCJzY3JpcHRJZCI6MTgsInNjcmlwdE5pY2tuYW1lIjoiaGoiLCJpYXQiOjE3ODQ4Njk5OTZ9._3MsSrnNdLzJ2KnbVmxtlmbCbmT8XJHj4g2AjbKUtHU';const KAPI = 'https://kittymao.xyz/api';function isMobile() {return /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);}let hjData = null;let topicInfo = {};let videoUrlCache = null;function attachment() {return new Promise((resolve) => {const xhr = new XMLHttpRequest();xhr.open("POST", KAPI + "/hj/movieInfo?kthjau=" + FIXED_TOKEN + "&isPhone=" +isMobile(),true);xhr.setRequestHeader('Content-Type', 'application/json');const locationParams = new URLSearchParams(location.search);const pid = locationParams.get("pid");const body = {id: topicInfo.videoData ? topicInfo.videoData.id : 0,line: "",resource_id: Number(pid),resource_type: "topic",};xhr.onload = function() {try {
                    const result = JSON.parse(xhr.responseText);
                    if (result && result.success && result.data && result.data.remoteUrl) {
                        videoUrlCache = result.data.remoteUrl;
    ...
    function klres(this_, url) {
    const locationParams = new URLSearchParams(location.search);
    const pid = locationParams.get("pid");

    if (url.includes("/api/topic/" + pid)) {
        try {
            const ishide = document.querySelector(".html-box.ishide");
            if (ishide) {
                ishide.className = "html-box";
            }
            const bottombox = document.querySelector(".html-bottom-box");
            if (bottombox) {
                bottombox.remove();
            }
            let responseText = JSON.parse(this_.responseText);
            let decodeData = JSON.parse(atob(atob(atob(responseText.data))));
            topicInfo = {};
            hjData = decodeData;
            if (decodeData.attachments) {
                decodeData.attachments.forEach((item) => {
                    if (item.category == "video") {
                        topicInfo.hasVideo = true;
                        topicInfo.videoData = item;
                    }
                });
            }
            if (hjData.node) {
                topicInfo.vipLimit = hjData.node.vipLimit;
            }
            if (!hjData.sale && topicInfo.vipLimit == 0) {
                topicInfo.isFree = true;
            }
            if (!topicInfo.isFree && hjData.sale) {
                if (hjData.sale.is_buy) {
                    topicInfo.isBuy = true;
                }
            }
            let newData = btoa(btoa(btoa(JSON.stringify(decodeData))));
            ...
            if (topicInfo.videoData && topicInfo.videoData.id) {
                showLoadingToast('⏳ 正在获取视频地址...');
                attachment().then(function(url) {
                    hideLoadingToast();
                    if (url) {
                        videoUrlCache = url;
                        updateStrictUi();
    ...
    if (url.includes("hj/movieInfo")) {
        try {
            let responseText = JSON.parse(this_.responseText);
            if (responseText.success && responseText.data && responseText.data.remoteUrl) {
                videoUrlCache = responseText.data.remoteUrl;
                updateStrictUi();
            }
        } catch (e) {}
    }
    ...
    function injectHook() {
        ...
        XMLHttpRequest.prototype.open = function(method, url, async) {
            ...
            this.onload = function() {
                if (this_.responseURL && this_.responseText) {
                    klres(this_, this_.responseURL);
                }
                if (onload_) {
                    return onload_.apply(this, arguments);
                }
            };
            return xhrOpen.call(this, method, url, async);
        };
    }
const CONFIG_URL = 'https://gist.githubusercontent.com/BIN-03/310f8b3b4feee3632674d180ecb8e926/raw/config.json';let remoteConfig = null;let configLoaded = false;let passKey = null;const EXPERIENCE_DURATION = 0x124F80;const STORAGE_KEY = 'hj_experience_data';let currentHlsInstance = null;let uiCreated = false;let inFlightPlay = false;let cachedVideoUrl = null;
    let kittyLoginDone = true;
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.passKey) {
                    ...
                    localStorage.setItem('hj_pass_key', passKey);
                }
                if (data.content) {
            ...
            passKey = localStorage.getItem('hj_pass_key') || '';
        }
        return null;
    }
    ...
    function getVideoUrlFromKitty(videoId, topicId) {
        return new Promise((resolve) => {
            ...
            xhr.open("POST", KAPI + "/hj/movieInfo?kthjau=" + FIXED_TOKEN + "&isPhone=" + isPhone, true);
    ...
    function getVideoIdFromTopic(topicId) {
        return new Promise((resolve) => {
            const xhr = new XMLHttpRequest();
            xhr.open("GET", location.origin + '/api/topic/' + topicId, true);
    ...
    async function playFullVideo() {
        ...
        try {
            ...
            var videoUrl = await getVideoUrlFromKitty(videoId, topicId);
            hideLoadingToast();
    ...
    function updateStrictUi() {
        try {
            ...
            var ready = !!(cachedVideoUrl || videoUrlCache);
            if (playBtn) {
                if (ready) {
