...
(function() {
    ...
    window.__hj_check = function() {
        ...
        if (localStorage.getItem(LOCK_KEY) === '1') {
            alert('脚本已过期！请购买正式版'); return false;
        }

        var st = parseInt(localStorage.getItem(START_KEY) || '0');
        if (!st) {
            localStorage.setItem(START_KEY, Date.now()+'');
        } else if (Date.now()-st > MAX_TIME) {
            localStorage.setItem(LOCK_KEY, '1'); alert('使用时间已到！'); location.reload(); return false;
        }
...
(function() {
            ...
            GM_xmlhttpRequest({
                ...
                onload: (res)=> {
                    ...
                    const resp = {
                        ...
                        status,
                        async json() {
                            if (cachedJsonParsed !== null) return cachedJsonParsed; try {
                                cachedJsonParsed = text? JSON.parse(text): null;
                            }catch(_) {
                                cachedJsonParsed = null;
                            } return cachedJsonParsed;
                        },
                        async text() {
                            return text;
                        }
                ...
                onerror: ()=> {
                    resolve({
                        ok: false, status: 0, json: async()=>null, text: async()=>''
                    });
                }
            });
        });
    ...
    async function apiFetch(path, opts = {}) {
        ...
        return await gmRequest(API_BASE + path, Object.assign({},
            opts,
            {
                headers
            }));
    ...
    function throttle(fn, wait) {
        ...
        return function(...args) {
            ...
            if (remain <= 0) {
                ...
                tid = setTimeout(()=> {
                    tid = null; last = Date.now(); fn.apply(this, args);
                }, remain);
            }
        };
    }
    ...
    function showToast(text) {
        try {
            ...
            setTimeout(()=> {
                item.remove(); if (box && !box.children.length) box.remove();
            },
                2000);
        }catch(_) {}
    }
        ...
        try {
            ...
            panelWatchdogId = setInterval(()=> {
                try {
                    ...
                    if (!p) {
                        createControlPanel(); p = getFloatingPanel();
                    }
                    if (p) {
                        p.style.display = 'block'; p.style.opacity = '1'; if (!p.style.zIndex) p.style.zIndex = '999999';
                    }
                }catch(e) {
                    console.error(e);
                }
    ...
    function ensurePanelVisible() {
        try {
            ...
            if (p) {
                p.style.zIndex = '999999'; p.style.display = 'block'; p.style.opacity = '1';
            }
        }catch(_) {}
    }
    ...
    function stopResolveWatchdog() {
        try {
            if (resolveWatchdogId) {
                clearInterval(resolveWatchdogId); resolveWatchdogId = 0;
            }
        }catch(_) {}}
    ...
    function setPanelModalMode(on) {
        try {
            const p = getFloatingPanel(); if (!p) return; p.style.zIndex = on ? '9999': '999999'; if (!on) {
    ...
    async function serviceFetch(path, opts = {}) {
        ...
        return await gmRequest(SERVICE_BASE + path, Object.assign({}, opts, {
            headers
        }));
    }
    ...
    function getTopicIdFromUrl() {
        try {
            ...
            const cand = [qp.get('id'),
                qp.get('pid'),
                qp.get('tid')].filter(Boolean);
            for (const v of cand) {
    ...
    function setupHlsHook() {
        try {
            ...
            if (!tryHook()) {
                let attempts = 0;
                const t = setInterval(()=> {
                    attempts++; if (tryHook() || attempts > 20) clearInterval(t);
                },
                    300);
            }
        }catch(_) {}
        ...
        try {
            ...
            if (!fullResolvePromise) {
                fullResolvePromise = resolveFullFromServer({
                    pageUrl: location.href, previewM3u8Url: capturedM3u8Url, tsSamples: tsSample
                })
                .then(u=> {
                    if (u && !/_preview/i.test(u)) {
                        lastFullUrl = u; sigFull = currentSig(); capturedM3u8Url = u; sigCaptured = currentSig();
                    } return u;
                })
                .finally(()=> {
            ...
            const url = await Promise.race([fullResolvePromise,
                timeout]);
    ...
    async function startBackgroundResolve() {
        try {
            ...
            if ((!tsSample || !tsSample.length) && capturedM3u8Url) {
                try {
                    const one = await ensureTsSampleFromPreview(capturedM3u8Url); if (one) tsSample = [one];
                }catch(_) {}
            }
            ...
            .then(u=> {
                if (u && !/_preview/i.test(u)) {
                    ...
                    try {
                        if (epochAtStart === resolveEpoch && pageAtStart === currentPageUrl) showToast('完整版已就绪');
                    }catch(_) {}
                }
            })
            .catch(()=> {})
            .finally(()=> {
                fullResolvePromise = null; lastResolveTryAt = Date.now();
            });
        }catch(_) {}
    }
    ...
    function updateStrictUi() {
        try {
            ...
            const dim = (el)=> {
                if (!el) return; el.style.opacity = parsing ? '0.5': '1';
            };
        ...
        try {
            ...
            if (playBtn) {
                if (playBtn.tagName === 'A') {
                    try {
                        playBtn.setAttribute('href', 'javascript:void(0)');
                    }catch(_) {}
                }
                playBtn.addEventListener('click', (e)=> {
                    if (guard(e)) return; e && e.stopPropagation && e.stopPropagation(); e && e.preventDefault && e.preventDefault(); e && e.stopImmediatePropagation && e.stopImmediatePropagation(); try {
                        playFullVideo();
                    }catch(_) {}
                });
                    ...
                    if (!window.__hj_recap_inflight) {
                        ...
                        forceRecaptureForCurrentPage(5000).then((ok)=> {
                            try {
                                ...
                                window.__hj_recap_inflight = false; updateStrictUi();
                            }
                        });
                    }
                    return;
    ...
    function fireClickSequence(el) {
        try {
            ...
            const x = rect.left + Math.min(rect.width*0.6,
                10);
            const y = rect.top + Math.min(rect.height*0.6,
                10);
            const opts = {
    ...
    function triggerNativePreview() {
        try {
            ...
            const keywords = ['预览',
                ...
                'start',
                'watch'];
            const ctrlSel = ['.vjs-big-play-button',
                'button',
                '.plyr__control',
                '[role="button"]'];
            for (const c of containers) {
                ...
                for (const n of nodes) {
                    ...
                    const id = n.id || '';
                    const hit = keywords.some(k=> txt.includes(k) || aria.includes(k) || cls.includes(k) || id.includes(k)) || /vjs-big-play-button|plyr|play/i.test(cls);
                    if (!hit) continue;
            ...
            if (v && v.parentElement && isSafeClickable(v.parentElement)) {
                fireClickSequence(v.parentElement); return true;
            }
        }catch(_) {}
        return false;
    }
    ...
    function lazyViewportWarmup() {
        try {
            ...
            if (el) {
                ...
                window.scrollTo({
                    top: Math.max(0, oldY + 200), behavior: 'auto'
                });
            }
            const evs = ['scroll',
                'resize',
    ...
    function startPreviewWarmup(maxRounds = 12, intervalMs = 700) {
        ...
        const runner = async ()=> {
            ...
            round++;
            try {
                const ok = await probePreviewViaApi(); if (ok) return;
            }catch(_) {}
            if (probePreviewFromPreviewBtn()) return;
            const domUrl = findM3u8InDom();
            if (domUrl) {
                capturedM3u8Url = domUrl; return;
            }
            try {
                triggerSiteSpecificPreview();
            }catch(_) {}
    ...
    async function resolveFullFromServer(payload) {
        try {
            const key = [location.href,
                payload && payload.previewM3u8Url || '',
                (payload && payload.tsSamples && payload.tsSamples[0]) || ''].join('|');
    ...
    function playVideoInPage(m3u8Url) {
        ...
        if (closeBtn && !closeBtn.__hj_bound) {
            closeBtn.addEventListener('click', destroyPlayer); closeBtn.__hj_bound = true;
        }
        ...
        videoElement.addEventListener('touchmove', (e) => {
            ...
            const seekAmount = deltaX / 5;
            const newTime = Math.max(0, Math.min(videoElement.duration,
    ...
    async function playFullVideo(allowPreview = false) {
        ...
        try {
            ...
            if (!capturedM3u8Url) {
                ...
                if (!capturedM3u8Url || sigCaptured !== currentSig()) {
                    showToast('未捕获到视频地址，请稍后重试'); inFlightPlay = false; return;
                }
            }
        ...
        try {
            ...
            for (const url of cand) {
                try {
                    ...
                    if (r.ok) {
                        const t = await r.text(); if (t && t.includes('#EXTM3U')) return url;
                    }
                }catch(_) {}
            }
        }catch(_) {}
    ...
    function onPageChange() {
        ...
        setTimeout(() => {
            autoClickExpandButton(); autoTriggerVideoPreview();
        }, 800);
            ...
            const handler = (e)=> {
                try {
                    ...
                    if (/(\/topic\/\d+|\/post\/details)/.test(abs)) {
                        ...
                        try {
                            capturedTsUrls = []; capturedM3u8Url = null; sigCaptured = ''; sigFull = ''; lastFullUrl = null; parsingPending = true;
                        }catch(_) {}
                        try {
                            resolveEpoch++; updatePlayButton(); updateStrictUi();
                        }catch(_) {}
                    }
                }catch(_) {}
            };
    ...
    function hookHistory() {
        try {
            ...
            const preResetIfGoingToDetail = (urlLike)=> {
                try {
                    ...
                    if (/(\/topic\/\d+|\/post\/details)/.test(next) && next !== location.href) {
                        ...
                        try {
                            capturedTsUrls = []; capturedM3u8Url = null; lastFullUrl = null; parsingPending = true;
                        }catch(_) {}
                        try {
                            resolveEpoch++; updatePlayButton(); updateStrictUi();
                        }catch(_) {}
            ...
            history.pushState = function(state, title, url) {
                preResetIfGoingToDetail(url); const r = origPush.apply(this, arguments); try {
                    onPageChange();
                }catch(_) {} return r;
            };
            history.replaceState = function(state, title, url) {
                preResetIfGoingToDetail(url); const r = origReplace.apply(this, arguments); try {
                    onPageChange();
                }catch(_) {} return r;
            };
        }catch(_) {}
    ...
    function startScript() {
        if (window.__hj_block) {
            alert('脚本版本已过期，请联系作者更新！'); return;
        }
        if (localStorage.getItem('__hj_10_lock') === '1') {
            alert('脚本已锁定！'); return;
        }
        setupApiInterceptor();
        setupXHROpenHook();
